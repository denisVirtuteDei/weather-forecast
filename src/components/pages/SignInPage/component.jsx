...
import { Button, Checkbox, Divider, FormControlLabel, Link, TextField } from '@material-ui/core'
...
import { signUpUserRequest, signInUserWithEmail, signInUserWithGoogle } from '@/actions/user'

import { isEqual, emailValidator, passwordLengthValidator } from '@/utils/validators'
...
const SingInPage = () => {
  ...
  const handleOnSubmitFormSignIn = e => {
    e.preventDefault()
    if (isSignUp && !isEmailValid && !isPasswordsEqual && !isPasswordLengthValid) {
    ...
    <CenteredImgDiv>
      <FormWrapper>
        ...
        <form className='form' onSubmit={handleOnSubmitFormSignIn}>
          ...
          />
          <TextField
            error={!!isPasswordLengthValid || (isSignUp ? !!isPasswordsEqual : false)}
            helperText={isPasswordLengthValid || (isSignUp ? isPasswordsEqual : '')}
            required
            fullWidth
            margin='normal'
            variant='outlined'
          ...
          {isSignUp ? (
            <TextField
              error={!!isPasswordLengthValid || !!isPasswordsEqual}
